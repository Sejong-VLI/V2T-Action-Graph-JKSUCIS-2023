...
class LinearProj(nn.Module):
    """Implementation of the linear projection layer for GATv2C.
        Params:
...
class GATv2C(nn.Module):
    """Implementation of the transformer heads module from the "How Attentive are Graph Attention Networks?"
        <https://arxiv.org/abs/2105.14491>` paper.
        Params:
            node_feat_dim: input node feature dimension.
    ...
    def __init__(self, node_feat_dim, d_model, edge_dim, heads=4, project_edge_dim=None, more_skip=True, last_average=False):
        ...
        self.conv1 = GATv2Conv(d_model, int(d_model/heads), heads, edge_dim=edge_dim, aggr='mean')
        
        self.conv2 = GATv2Conv(d_model, int(d_model/heads), heads, edge_dim=edge_dim, aggr='mean')
        
        if last_average:
            self.conv3 = GATv2Conv(d_model, d_model, heads, concat=False, edge_dim=edge_dim, aggr='mean')
        else:
            self.conv3 = GATv2Conv(d_model, int(d_model/heads), heads, edge_dim=edge_dim, aggr='mean')
